...
if ($data['action'] == 'update') {
    ...
    $record_date = date("Y-m-d");
    $query = $con->prepare("UPDATE tbl_records SET patient_id = ?, doctor_id = ?, diagnosis = ?, treatment = ?, record_date = ? WHERE record_id = ?");
    $query->bind_param("iisssi", $patient_id, $doctor_id, $diagnosis, $treatment, $record_date, $record_id);
    $query->execute();
    if ($query->error) {
        $response["status"] = false;
...
if ($data['action'] == 'delete') {
    $record_id = $data["data"]["record_id"];
    $query = $con->prepare("DELETE FROM tbl_records WHERE record_id = ?");
